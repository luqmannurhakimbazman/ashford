...
Retain `syllabus_source_ingested` and `syllabus_approval_recorded` as read-only legacy v1 event variants so existing 2.1 ledgers rebuild unchanged. Stop emitting them. Move the ST adapter and assertion manifest out of the production dispatch path and into test/evidence fixtures; a small legacy validator may retain their old schema without exposing a live ingestion adapter.
