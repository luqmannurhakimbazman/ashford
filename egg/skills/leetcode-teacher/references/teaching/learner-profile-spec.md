...
## File 1: `${CLAUDE_PLUGIN_DATA}/leetcode-teacher-profile.md` (Working Memory)
...
## File 2: `${CLAUDE_PLUGIN_DATA}/leetcode-teacher-ledger.md` (Long-Term Record)
...
All timestamps use: `YYYY-MM-DDTHH:MM` (e.g., `2026-02-19T14:30`). No seconds, no timezone. Source: `Session Timestamp` from `=== SESSION METADATA ===` (generated by the `leetcode-profile-sync` agent), or from `${CLAUDE_PLUGIN_DATA}/leetcode-session-state.md` (after compaction), or via `date +%Y-%m-%dT%H:%M` bash fallback if neither is available.
...
2. `Session ID` from `${CLAUDE_PLUGIN_DATA}/leetcode-session-state.md` (after compaction — may be the platform `session_id` from hook JSON if the agent's UUID wasn't persisted)
3. **Fallback:** `manual`
...
2. `Session Timestamp` from `${CLAUDE_PLUGIN_DATA}/leetcode-session-state.md` (after compaction)
...
1. **Append row to Ledger** (`${CLAUDE_PLUGIN_DATA}/leetcode-teacher-ledger.md`):
...
2. **Append to Session History** in profile (`${CLAUDE_PLUGIN_DATA}/leetcode-teacher-profile.md`), newest first. Enforce 20-entry cap by removing the oldest entry if needed. Use the semi-structured format:
   ```
