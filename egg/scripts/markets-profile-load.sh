#!/bin/bash
# SessionStart hook: Load, validate, and sync-heal the markets teacher profile.
...
PROFILE="$DATA_DIR/markets-teacher-profile.md"
LEDGER="$DATA_DIR/markets-teacher-ledger.md"
...
migrate_legacy_file "markets-teacher-profile.md"
migrate_legacy_file "markets-teacher-ledger.md"
migrate_legacy_file "markets-session-state.md"
...
rmdir "$GUARD_ROOT/markets-$SESSION_KEY" 2>/dev/null || true
...
if ! grep -q "^## About Me" "$PROFILE" 2>/dev/null; then
  ...
  echo '' >> "$TMPFILE"
  echo '<!-- Write your goals, target firms, asset class focus, preferred level, and timeline here. -->' >> "$TMPFILE"
  echo '' >> "$TMPFILE"
  cat "$PROFILE" >> "$TMPFILE"
  mv "$TMPFILE" "$PROFILE"
...
if [ -n "$LAST_PROFILE_TS" ] && [ -n "$LAST_LEDGER_TS" ]; then
  if [[ "$LAST_PROFILE_TS" > "$LAST_LEDGER_TS" ]]; then
    ...
    P_TOPIC=$(echo "$ENTRY_LINE" | cut -d'|' -f2 | xargs)
    P_ASSET=$(echo "$ENTRY_LINE" | cut -d'|' -f3 | xargs)
    P_MODE=$(echo "$ENTRY_LINE" | cut -d'|' -f4 | xargs)
    P_VERDICT=$(echo "$ENTRY_LINE" | cut -d'|' -f5 | xargs)
    ...
    append_ledger_row "$LAST_PROFILE_TS" "sync-heal" "$P_TOPIC" "$P_ASSET" "$P_MODE" "$P_VERDICT" "$P_GAPS" "$P_REVIEW"
    REPAIRS="${REPAIRS}[SYNC] Appended missing ledger row for ${LAST_PROFILE_TS} session (asset_class=${P_ASSET}).\n"
  fi
...
# --- Output full profile ---
echo "=== MARKETS PROFILE (loaded automatically at session start) ==="
cat "$PROFILE"
echo ""
echo "=== END MARKETS PROFILE ==="
