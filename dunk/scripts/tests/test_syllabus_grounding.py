...
from dln_store.schema import (
    ValidationError,
    canonical_json,
    sha256_bytes,
    syllabus_approval_set_sha256,
)
...
def tree(directory: Path) -> dict[str, bytes]:
    return {
        ...
        if p.is_file() and ".dln-transaction" not in p.parts
    }
...
def prepared(
    text: str = "Course: Generic Systems\nTopic: Trees\nExam date: TBA\n",
    *,
    ...
    return {
        ...
        "normalization": {
            "policy_id": "fixture-normalization-v1",
            "unicode": "NFC",
            "line_endings": "LF",
        },
        "units": [
...
def lifecycle(
    ...
    *,
    ...
    source = store._install_prepared_syllabus(
        ...
        base_minute * 3,
        raw_bytes=raw,
        prepared_document=prepared(text),
        role=role,
        display_name="generic-syllabus.pdf",
        occurred_at=f"2026-08-19T00:{base_minute:02d}:00Z",
    ...
    proposals = [
        ...
        located(
            text,
            "TBA",
            predicate="assessment.exam.date",
            label="Exam date",
            semantic_roles=["assessment"],
            value_type="date",
...
def test_prepare_propose_decide_cas_content_and_offline_rebuild(tmp_path: Path) -> None:
    ...
    assert (
        directory.joinpath("sources", "sha256", source_digest).read_bytes() == b"generic-source-v1"
    )
    prepared_bytes = directory.joinpath(
        "prepared", "sha256", f"{prepared_digest}.json"
    ).read_bytes()
    ...
    assert grounding["planning_topics"] == [
        {"assertion_ids": [proposal["proposal_ids"][0]], "citable": True, "label": "Trees"}
    ]
    before_cas = {
        p.relative_to(directory).as_posix(): p.read_bytes()
        ...
        p.relative_to(directory).as_posix(): p.read_bytes()
        for p in (directory / "prepared").rglob("*")
        if p.is_file()
    }
...
def test_prepare_propose_decide_exact_retries_are_idempotent(tmp_path: Path) -> None:
    ...
    source, proposal, decision = lifecycle(store, domain_id)
    text = "Course: Generic Systems\nTopic: Trees\nExam date: TBA\n"
    retry_source = store._install_prepared_syllabus(
        domain_id,
        3,
        raw_bytes=b"generic-source-v1",
        prepared_document=prepared(text),
        role="authoritative",
        display_name="retry-name.pdf",
        occurred_at="2026-08-19T00:59:00Z",
    )
    assert retry_source["status"] == "noop" and retry_source["revision"] == 3
    retry_proposal = store.propose_syllabus(
        domain_id,
        3,
        prepared_event_id=source["source_event_id"],
        occurred_at="2026-08-19T00:58:00Z",
        producer={"trust": "external_unverified", "name": "test-builder", "version": "1"},
        proposals=[
            located(
                text,
    ...
    assert (
        retry_proposal["status"] == "noop"
        and retry_proposal["proposal_event_id"] == proposal["proposal_event_id"]
    )
    retry_decision = store.decide_syllabus(
        domain_id,
        3,
        proposal_event_id=proposal["proposal_event_id"],
        occurred_at="2026-08-19T00:57:00Z",
        accepted_proposal_ids=proposal["proposal_ids"],
        deferred_proposal_ids=[],
        rejected_proposal_ids=[],
        corrections=[],
    ...
    assert (
        retry_decision["status"] == "noop"
        and retry_decision["decision_event_id"] == decision["decision_event_id"]
    )
...
def test_existing_nonmatching_cas_path_is_never_overwritten(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError, match="collision/corruption"):
        store._install_prepared_syllabus(
            ...
            0,
            raw_bytes=raw,
            prepared_document=prepared("Topic: Trees\n"),
            role="authoritative",
...
def test_locator_and_decision_failures_are_write_free(tmp_path: Path) -> None:
    ...
    ambiguous = [
        located(
            text,
            "A or B",
            predicate="class.room",
            label="Room",
            semantic_roles=["logistics"],
            value_type="text",
            status="ambiguous",
            value=None,
            ambiguity={
                "reason": "two rooms listed",
                "unresolved_dimensions": ["room"],
                "candidates": [],
            },
        )
    ]
    ...
    store.decide_syllabus(
        ...
        corrections=[
            {
                "target_proposal_id": proposal["proposal_ids"][0],
                "predicate": "class.room",
                "semantic_roles": ["logistics"],
                "value_type": "text",
                "status": "specified",
                "value": "A",
                "rationale": "Learner confirmed room A",
            ...
            corrections=[
                {
                    ...
                    "status": "specified",
                    "value": 7,
                    "rationale": "Adversarial non-text topic",
                    "origin": "learner_correction",
                }
            ],
        )
...
def test_generic_and_legacy_correction_ids_cannot_collide_with_source_assertions() -> None:
    ...
    generic = {
        "accepted_proposal_ids": [],
        "deferred_proposal_ids": ["source-b"],
        "rejected_proposal_ids": [],
    ...
    legacy = {
        ...
        "deferred_assertion_ids": ["source-b"],
    ...
    for decision, is_legacy in ((generic, False), (legacy, True)):
        with pytest.raises(
            ValidationError, match="must not collide with proposal/source assertion IDs"
        ):
...
def test_authoritative_update_pending_and_nonforking_lineage(tmp_path: Path) -> None:
    ...
    second = store._install_prepared_syllabus(
        domain_id,
        3,
        raw_bytes=b"generic-source-v2",
        prepared_document=prepared("Topic: Graphs\n"),
        role="authoritative",
        display_name="update.pdf",
        occurred_at="2026-08-19T00:03:00Z",
        supersedes_source_version_id=first["source_version_id"],
    )
    grounding = store.context(domain_id)["state"]["grounding"]
    ...
    with pytest.raises(ValidationError, match="forks/skips"):
        store._install_prepared_syllabus(
            ...
            4,
            raw_bytes=b"fork",
            prepared_document=prepared("Topic: Fork\n"),
            role="authoritative",
            display_name="fork.pdf",
...
def test_supplement_visible_but_authority_planning_and_mastery_neutral(tmp_path: Path) -> None:
    ...
    supplement, proposal, supplement_decision = lifecycle(
        store, domain_id, role="supplement", raw=b"supplement", base_minute=1
    )
    ...
    assert (
        after["grounding"]["supplements"][0]["source_version_id"] == supplement["source_version_id"]
    )
    mastery_keys = ["stage", "generation", "subjects", "calibration", "archived_exams"]
    assert canonical_json({key: before[key] for key in mastery_keys}) == canonical_json(
        {key: after[key] for key in mastery_keys}
    )
...
def test_missing_corrupt_and_orphaned_cas_are_integrity_failures(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError) as combined:
        store.validate(domain_id)
    assert "missing canonical" in str(combined.value) and "orphaned canonical" in str(
        combined.value
    )
...
def test_chronology_admin_session_and_payload_bounds_are_enforced(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError, match="cannot precede"):
        store.propose_syllabus(
            ...
            1,
            prepared_event_id=source["source_event_id"],
            occurred_at="2026-08-19T00:09:00Z",
            producer={"trust": "external_unverified", "name": "test", "version": "1"},
            proposals=proposal_input,
        )
    assert tree(directory) == before
    oversized = deepcopy(proposal_input)
    oversized[0]["note"] = "x" * 4097
    with pytest.raises(ValidationError, match="4096"):
        store.propose_syllabus(
            domain_id,
            1,
            prepared_event_id=source["source_event_id"],
            occurred_at="2026-08-19T00:11:00Z",
            producer={"trust": "external_unverified", "name": "test", "version": "1"},
            proposals=oversized,
        )
    proposal = store.propose_syllabus(
    ...
    with pytest.raises(ValidationError, match="cannot precede"):
        store.decide_syllabus(
            ...
            2,
            proposal_event_id=proposal["proposal_event_id"],
            occurred_at="2026-08-19T00:10:30Z",
            accepted_proposal_ids=proposal["proposal_ids"],
            deferred_proposal_ids=[],
            rejected_proposal_ids=[],
            corrections=[],
        )
    admin_session = json.loads(directory.joinpath("events.jsonl").read_text().splitlines()[0])[
        "session_id"
    ]
    reset = {
        "schema_version": 1,
        "event_id": "bad-admin-reuse",
...
def test_legacy_unresolved_correction_remains_uncitable() -> None:
    ...
    approval["corrections"] = [
        {
            "correction_assertion_id": "legacy-unresolved-correction",
            "target_assertion_id": "legacy-topic-graphs",
            "field": "coverage.topic",
            "status": "unresolved",
            "normalized_value": {"possibilities": ["Graphs", "Trees"]},
            "rationale": "historical ambiguity",
            "origin": "learner_correction",
        }
    ]
...
def test_rejected_proposals_are_visible_but_not_citable(tmp_path: Path) -> None:
    ...
    store.decide_syllabus(
        ...
        accepted_proposal_ids=[],
        deferred_proposal_ids=[],
        rejected_proposal_ids=proposal["proposal_ids"],
        corrections=[],
    )
    ...
    assert (
        ...
        and proposal["proposal_ids"][0] in rendered
        and "Rejected:" in rendered
    )
...
def test_legacy_v1_fixture_replays_without_pdf_manifest_or_cas(tmp_path: Path) -> None:
    ...
    source_event_id = json.loads(fixture.joinpath("events.jsonl").read_text().splitlines()[0])[
        "event_id"
    ]
