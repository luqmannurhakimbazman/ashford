...
from .schema import (
    ...
    ValidationError,
    validate_event,
    validate_locator_against_document,
)
...
def _source_summary(source: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "ingested_at": source["occurred_at"],
        ...
        "phase": source.get("phase", "prepared"),
    }
...
def _proposal_effective(proposal: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "semantic_roles": deepcopy(proposal["semantic_roles"]),
        "value_type": proposal["value_type"],
        "status": proposal["status"],
        "value": deepcopy(proposal["value"]),
        "normalized_value": deepcopy(proposal["value"]),
        "origin": "document",
...
def _corrected_effective(correction: dict[str, Any], target: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "source_assertion_id": target["proposal_id"],
        "predicate": correction.get("predicate", correction.get("field")),
        ...
        "document_context": deepcopy(target["locators"]),
        "citations": [],
        "document_status": target["status"],
        "document_value": deepcopy(target["value"]),
    }
    ...
    *,
    ...
    if correction_ids & proposal_ids:
        raise ValidationError(
            "syllabus correction IDs must not collide with proposal/source assertion IDs"
        )
    ...
    return {
        ...
        "rejected_assertions": rejected_entries,
...
class GroundingTimeline:
    """Reduced syllabus authority: source/proposal/decision lineage and resolvable citations."""
    ...
    def current_view(self) -> dict[str, Any] | None:
        """Return the active authoritative decision view, or None while ungrounded."""
        return (
            self.approval_views.get(self.current_approval_event_id)
            if self.current_approval_event_id
            else None
        )

    def resolve_assertion(self, authority_event_id: str, assertion_id: str) -> dict[str, Any]:
        """Return one settled effective assertion, rejecting unknown authority or citation IDs."""
        view = self.approval_views.get(authority_event_id)
        if view is None:
            raise ValidationError(
                f"unknown syllabus decision/approval event {authority_event_id!r}"
            )
        assertion = view["eligible_by_id"].get(assertion_id)
        if assertion is None:
            raise ValidationError(
                f"assertion {assertion_id!r} was not settled and effective "
                f"in authority {authority_event_id!r}"
            )
        return deepcopy(assertion)

    def projected_state(self, legacy_syllabus: list[str]) -> dict[str, Any]:
        """Build the bounded `state.grounding` bundle, falling back to legacy flat topics."""
        ...
        if current:
            ...
            active_decision = {
                ...
                "decision_set_sha256": current["decision_digest"],
        ...
        for version in self.source_order:
            if active_version is None or self.source_order.index(version) > self.source_order.index(
                active_version
            ):
        ...
        if not self.source_order:
            ...
            status = (
                "decision_required" if latest.get("phase") == "proposed" else "proposal_required"
            )
        elif pending:
        ...
        if current:
            ...
            for assertion in current["effective_assertions"]:
                if (
                    "planning_topic" not in assertion.get("semantic_roles", [])
                    or assertion["status"] != "specified"
                    or not isinstance(assertion["value"], str)
                ):
                    continue
                topic = by_label.setdefault(
                    assertion["value"],
                    {"assertion_ids": [], "citable": True, "label": assertion["value"]},
                )
        ...
        supplements = [
            _source_summary(self.sources_by_version[version]) for version in self.supplement_order
        ]
        return {
            "active_approval": active_decision,
            "active_decision": active_decision,
            "active_source": active_source,
            "effective_assertions": effective,
            "legacy_fallback": legacy_fallback,
            "pending_sources": pending,
            "pending_authoritative_sources": pending,
...
def _check_locators(proposals: list[dict[str, Any]], document: dict[str, Any], prefix: str) -> None:
    for p_index, proposal in enumerate(proposals):
        for l_index, locator in enumerate(proposal["locators"]):
            validate_locator_against_document(
                locator, document, f"{prefix}[{p_index}].locators[{l_index}]"
            )
        for c_index, candidate in enumerate(proposal.get("ambiguity", {}).get("candidates", [])):
            for l_index, locator in enumerate(candidate["locators"]):
                validate_locator_against_document(
                    locator,
                    document,
                    f"{prefix}[{p_index}].ambiguity.candidates[{c_index}].locators[{l_index}]",
                )


def reduce_grounding_timeline(
    events: list[dict[str, Any]], prepared_documents: dict[str, dict[str, Any]] | None = None
) -> GroundingTimeline:
    ...
    for position, raw_event in enumerate(events):
        ...
        if event["event_id"] in seen_event_ids:
            raise ValidationError(
                f"events[{position}].event_id: duplicate event ID {event['event_id']!r}"
            )
        kind = event["kind"]
        if kind in RESERVED_SYLLABUS_EVENT_KINDS:
            ...
            raise ValidationError(
                f"events[{position}].session_id: non-syllabus events must not reuse "
                "a syllabus administrative session"
            )

        if kind in {"assessment", "session_completed"}:
            if event["session_id"] in admin_sessions:
                raise ValidationError(
                    f"events[{position}].session_id: learning events must not reuse "
                    "a syllabus administrative session"
                )
            ...
            if grounding is not None:
                if latest_authority_event is None:
                    raise ValidationError(
                        f"events[{position}].grounding: no syllabus decision was "
                        "active before this event"
                    )
                key = (
                    "approval_event_id" if "approval_event_id" in grounding else "decision_event_id"
                )
                if grounding[key] != latest_authority_event["event_id"]:
                    raise ValidationError(
                        f"events[{position}].grounding.{key}: expected active authority "
                        f"{latest_authority_event['event_id']!r}"
                    )
                ...
                if key != reference_field and not (
                    ...
                    raise ValidationError(
                        f"events[{position}].grounding: authority reference key does "
                        "not match authority kind"
                    )
                ...
                if used < _instant(latest_authority_event["occurred_at"]):
                    raise ValidationError(
                        f"events[{position}].grounding: event precedes active authority"
                    )
                latest_grounded_use[grounding[key]] = max(
                    used, latest_grounded_use.get(grounding[key], used)
                )
        ...
        if kind in {"syllabus_source_ingested", "syllabus_source_prepared"}:
            ...
            if legacy:
                ...
                source = {
                    ...
                    "sha256": digest,
                    "display_name": raw_source["original_filename"],
                    "media_type": raw_source["media_type"],
                    "storage": "legacy_text_only",
                    "document": document,
                    "prepared_document_sha256": None,
                    "phase": "proposed",
                    "raw_event": event,
                    "proposals": legacy_proposals,
                    "proposal_digest": event["assertion_set_sha256"],
                }
            else:
                raw_source = event["source"]
                role, version, digest = (
                ...
                document = (
                    supplied.get("document")
                    if isinstance(supplied, dict) and "document" in supplied
                    else supplied
                )
                if document is None:
                    raise ValidationError(
                        f"events[{position}].prepared: verified prepared content is unavailable"
                    )
                source = {
                    "event_id": event["event_id"],
                    "occurred_at": event["occurred_at"],
                    ...
                    "storage": "cas",
                    "document": document,
                    "prepared_document_sha256": event["prepared"]["prepared_document_sha256"],
                    "phase": "prepared",
                    "raw_event": event,
                }
            ...
            if role == "authoritative":
                if latest_authoritative is None and predecessor is not None:
                    raise ValidationError(
                        f"events[{position}]: first authoritative source must not supersede another"
                    )
                if (
                    latest_authoritative is not None
                    and predecessor != latest_authoritative["source_version_id"]
                ):
                    raise ValidationError(
                        f"events[{position}]: authoritative source must supersede the "
                        "latest source; forks/skips are not allowed"
                    )
                if latest_authoritative and _instant(event["occurred_at"]) <= _instant(
                    latest_authoritative["occurred_at"]
                ):
                    raise ValidationError(
                        f"events[{position}].occurred_at: must be later than predecessor"
                    )
                ...
                if predecessor is not None:
                    raise ValidationError(
                        f"events[{position}]: supplements cannot supersede authoritative sources"
                    )
            ...
            if legacy:
                timeline.proposals_by_source_event[event["event_id"]] = {
                    "event": event,
                    "proposals": source["proposals"],
                    "digest": source["proposal_digest"],
                }
            ...
            if source is None or source["storage"] != "cas":
                raise ValidationError(
                    f"events[{position}].prepared_event_id: unknown prepared source"
                )
            if (
                event["role"] != source["role"]
                or event["source_version_id"] != source["source_version_id"]
                or event["prepared_document_sha256"] != source["prepared_document_sha256"]
            ):
                raise ValidationError(
                    f"events[{position}]: proposal pins do not match prepared source"
                )
            if source["role"] == "authoritative" and source is not latest_authoritative:
                raise ValidationError(
                    f"events[{position}]: proposals must target latest authoritative source"
                )
            if source["event_id"] in timeline.proposals_by_source_event:
                raise ValidationError(
                    f"events[{position}]: prepared source already has a proposal set"
                )
            if _instant(event["occurred_at"]) < _instant(source["occurred_at"]):
                raise ValidationError(
                    f"events[{position}].occurred_at: proposals cannot precede source preparation"
                )
            _check_locators(event["proposals"], source["document"], f"events[{position}].proposals")
            timeline.proposals_by_source_event[source["event_id"]] = {
                "event": event,
                "proposals": event["proposals"],
                "digest": event["proposal_set_sha256"],
            }
            ...
            if legacy:
                ...
                proposal_bundle = timeline.proposals_by_source_event.get(
                    source["event_id"] if source else ""
                )
                ...
                if source and (
                    event["source_version_id"] != source["source_version_id"]
                    or event["prepared_document_sha256"] != source["prepared_document_sha256"]
                    or event["role"] != source["role"]
                ):
                    raise ValidationError(f"events[{position}]: decision pins do not match source")
                if proposal_bundle and (
                    event["proposal_event_id"] != proposal_bundle["event"]["event_id"]
                    or event["proposal_set_sha256"] != proposal_bundle["digest"]
                ):
                    raise ValidationError(
                        f"events[{position}]: decision pins do not match proposal set"
                    )
            if source is None or proposal_bundle is None:
                raise ValidationError(
                    f"events[{position}]: decision references unknown source/proposals"
                )
            proposal_event = proposal_bundle["event"]
            if _instant(event["occurred_at"]) < _instant(source["occurred_at"]) or _instant(
                event["occurred_at"]
            ) < _instant(proposal_event["occurred_at"]):
                raise ValidationError(
                    f"events[{position}].occurred_at: decision cannot precede source/proposal"
                )
            proposals = proposal_bundle["proposals"]
            expected = {item["proposal_id"] for item in proposals}
            if legacy:
                decided = (
                    set(event["accepted_assertion_ids"])
                    | set(event["deferred_assertion_ids"])
                    | {item["target_assertion_id"] for item in event["corrections"]}
                )
            else:
                decided = (
                    set(event["accepted_proposal_ids"])
                    | set(event["deferred_proposal_ids"])
                    | set(event["rejected_proposal_ids"])
                    | {item["target_proposal_id"] for item in event["corrections"]}
                )
            if decided != expected:
                raise ValidationError(
                    f"events[{position}]: decision must completely and exactly "
                    "partition proposal IDs"
                )
            by_id = {item["proposal_id"]: item for item in proposals}
            corrections = event["corrections"]
            for correction in corrections:
                target_id = correction.get(
                    "target_proposal_id", correction.get("target_assertion_id")
                )
                target = by_id[target_id]
                if correction.get("predicate", correction.get("field")) != target["predicate"]:
                    raise ValidationError(
                        f"events[{position}].corrections: correction predicate must match target"
                    )
                if not legacy and correction["semantic_roles"] != target["semantic_roles"]:
                    raise ValidationError(
                        f"events[{position}].corrections: correction semantic roles "
                        "must match target"
                    )
            ...
            if source["role"] == "authoritative":
                if source is not latest_authoritative:
                    raise ValidationError(
                        f"events[{position}]: decision must target latest authoritative source"
                    )
                if latest_authority_event is None:
                    if predecessor is not None:
                        raise ValidationError(
                            f"events[{position}]: first authoritative decision must "
                            "not supersede another"
                        )
                elif predecessor != latest_authority_event["event_id"]:
                    raise ValidationError(
                        f"events[{position}]: decision must supersede active authority; "
                        "forks are not allowed"
                    )
                if latest_authority_event and _instant(event["occurred_at"]) <= _instant(
                    latest_authority_event["occurred_at"]
                ):
                    raise ValidationError(
                        f"events[{position}].occurred_at: must be later than superseded decision"
                    )
                last_use = (
                    latest_grounded_use.get(latest_authority_event["event_id"])
                    if latest_authority_event
                    else None
                )
                if last_use and _instant(event["occurred_at"]) <= last_use:
                    raise ValidationError(
                        f"events[{position}].occurred_at: must follow grounded uses "
                        "of prior authority"
                    )
                ...
                if predecessor != prior_id:
                    raise ValidationError(
                        f"events[{position}]: supplement decision predecessor must "
                        "remain within its supplement"
                    )
