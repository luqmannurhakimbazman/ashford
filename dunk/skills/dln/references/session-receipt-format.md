...
The receipt is generated from `session_completed.evidence_event_ids`. Before closing the session, ensure that list includes every same-session assessment, model revision, and stage transition the learner should see. It must not cite plan text, dialogue, profile patches, or events from another session.
