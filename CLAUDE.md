...
| `dunk/` | 2.2.0 | local-first, DLN-inspired tutoring with portable syllabus grounding and Obsidian-readable projections |
...
Dunk registers no hooks. Its local CLI enforces persistence invariants directly; do not add a remote-write hook or make an MCP service authoritative. Version 2.2.0 adds generic bounded PDF/HTML prepare, content, proposal, and learner-decision flows; content-addressed source/prepared retention; bounded grounding citations; and generated Syllabus Intake Receipts. PDF extraction is pinned to `pypdf==6.14.2`; OCR is not supported.
